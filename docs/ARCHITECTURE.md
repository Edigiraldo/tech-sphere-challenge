...
| `metrics/` | Observes latency (P50/P95), token consumption, model invocations, RAG queries, estimated cost. The collector module feeds a read-only typed ``GET /metrics`` endpoint and a metrics frontend view. The collector and the reporting API are distinct concerns. | Non-blocking observer. Never modifies application behavior. |
...
| 8 | Metrics API and polish | Metrics endpoint, README metrics, edge cases, all gates passable | ✅ Complete — read-only typed ``GET /metrics`` endpoint and metrics frontend view; metrics collector module is distinct from the reporting API; summaries generator exists |
