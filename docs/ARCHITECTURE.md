...
| `metrics/` | Observa latencia (P50/P95), consumo de tokens, invocaciones del modelo, consultas RAG y costo estimado únicamente del LLM. El módulo colector alimenta endpoints tipados de solo lectura ``GET /metrics/summary``, ``GET /metrics/calls`` y ``GET /metrics/calls/{call_id}`` más una vista frontal de métricas. | Observador no bloqueante. STT, TTS, embeddings, hosting y red quedan fuera del costo; nunca modifica el comportamiento de la aplicación. |
